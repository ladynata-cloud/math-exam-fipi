...
const CHECK = {
  ...
  "komb-06"(p) {
    if (!/вписан в цилиндр/.test(p.cond)) errs.push(p.id + ": в cond нет «вписан в цилиндр»");
    const v = grab(p, /[Шш]ар объёмом (\d+(?:,\d+)?)/);
  ...
  "komb-08"(p) {
    /* общие основание и высота заданы описанием: основания совпадают, вершина — в центре другого основания */
    if (!/основания совпадают/.test(p.cond) || !/в центре другого основания/.test(p.cond))
      errs.push(p.id + ": в cond не сказано, что основания общие и вершина конуса в центре другого основания");
    const v = grab(p, /[Цц]илиндр объёмом (\d+(?:,\d+)?)/);
