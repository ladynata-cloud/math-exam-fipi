...
Подсказка ничего не отнимает (26.09.2026, `docs/tasks/RIGHTTRI_HINT_KEEPS_CREDIT.md`):
в «Сам» после «С чего начать?» верный ответ с первой попытки растит `correct`
и `streak` так же, как без подсказки (раньше шёл только в `solved`, и
«чистые серии» на карточке курса не росли). Обращение к подсказке считается
отдельно — поле `hinted` в записи темы `righttri-t1` (старые записи без него
читаются как 0), учитель видит его в сводке «Сдать учителю». Фраза «Подсказка
ничего не отнимает…» стоит рядом с кнопкой и в раскрытой подсказке, справка
режима «Сам» тоже об этом говорит. `?selftest=1` больше не записывает
миграцию `pt-trainer-v1` в ключ курса — только в память.
По ревью (тот же день): фраза набрана цветом `--ink-soft` (контраст 6:1
вместо 3:1 у `--ink-faint`); при печати кнопка «С чего начать?» и раскрытая
подсказка скрыты вместе с остальной навигацией; подпись обещает в «Пошагово»
«задачу того же типа», а не «ту же» (смена режима строит новую задачу);
у задач, где первый шаг — `info` без подсказки (`t3-diff`, `t4-findAcute`),
«С чего начать?» показывает текст шага — обозначение неизвестного, не ответ.
