...
const storageKey = 'mathExamOge2027Analogue1.v1';
...
const evidence = { surfaces: [], targetsMeasured: 0, svgMeasurements: 0, printableTasks: 0, evidenceDir };
...
async function context(options, corruptStorage, denyStorage = false) {
  ...
  await value.addInitScript(({ key, corrupt, deny }) => {
  ...
  }, { key: storageKey, corrupt: corruptStorage, deny: denyStorage });
  return value;
}
...
async function flows(page, target, label, touch) {
  ...
  assert.equal((await score(target)).part1, 2, 'Revealed answer must not earn Part 1 credit');
