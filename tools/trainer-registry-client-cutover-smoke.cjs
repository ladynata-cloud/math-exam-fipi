...
async function gotoBoard(page, siteOrigin, boardOrigin, path = '/trainers/trainer-board.html') {
  ...
  await page.waitForSelector('#trainerFrame', { timeout: 60000 });
  await page.waitForFunction(() => document.querySelector('#trainerFrame')?.getAttribute('src') !== 'about:blank');
}
...
async function trainerFrame(page, suffix) {
  await page.waitForFunction(expected => [...document.querySelectorAll('iframe')].some(frame => frame.src.includes(expected)), suffix);
...
async function createRoom(page) {
  ...
  await page.waitForFunction(() => document.querySelector('#studentLink')?.value.includes('role=student'));
...
async function registryValidationMatrix(browser, siteOrigin, boardOrigin, registry) {
  ...
  for (const [name, response, expectedCalls, succeeds] of cases) {
    ...
    const evidence = instrument(page, name, ['TRAINER_REGISTRY_SHADOW_MISMATCH']);
    ...
    if (succeeds) {
      await page.waitForFunction(() => mirrorTrainerFromUrl('negative-numbers-line.html') === 'negative-numbers-line', null, { timeout: 12000 });
    } else {
  ...
  const emptyEvidence = instrument(emptyPage, 'empty-registry', ['TRAINER_REGISTRY_SHADOW_MISMATCH']);
...
async function canonicalPathMatrix(browser, siteOrigin, boardOrigin, otherOrigin, registry) {
  ...
  await installRegistryRoute(page, siteOrigin, control);
  await gotoBoard(page, siteOrigin, boardOrigin);
  await page.waitForFunction(() => mirrorTrainerFromUrl('negative-numbers-line.html') === 'negative-numbers-line');
  const matrix = [
...
async function arrivalAndFailClosed(browser, siteOrigin, boardOrigin, registry) {
  const context = await browser.newContext();
  const page = await context.newPage();
  const evidence = instrument(page, 'arrival-fail-closed', ['TRAINER_REGISTRY_SHADOW_MISMATCH']);
  const frameRequests = [];
  ...
  await page.waitForFunction(() => document.querySelectorAll('#trainerQuick option').length >= 12);
...
async function arrivalOrder(browser, siteOrigin, boardOrigin, registry, mode) {
  const context = await browser.newContext();
  const page = await context.newPage();
  const evidence = instrument(page, `arrival-${mode}`, ['TRAINER_REGISTRY_SHADOW_MISMATCH']);
  const control = { calls: 0, registry, mode: mode === 'ready-first' ? { delay: 1200, body: registry } : { body: registry } };
  await installRegistryRoute(page, siteOrigin, control);
...
async function liveTeacherStudent(browser, siteOrigin, boardOrigin, registry) {
  ...
  await studentLinear.waitForFunction(() => window.getBoardTrainerState().currentStep === 1);
  ...
  await student.waitForFunction(() => document.body.classList.contains('control-owner'));
  await studentLinear.fill('#val', '7');
  await teacherLinear.waitForFunction(() => document.querySelector('#val').value === '7');
  await studentLinear.evaluate(() => { const input = document.querySelector('#val'); input.value = '88'; input.dispatchEvent(new Event('input', { bubbles: true })); });
  await teacher.click('#controlToggle');
  await student.waitForFunction(() => !document.body.classList.contains('control-owner'));
  ...
  await teacher.waitForFunction(() => document.querySelector('#shareStatus')?.textContent === 'подключено');
  await waitMirror(teacher, 'connected');
  ...
  await reloadedTeacherLinear.waitForFunction(() => document.querySelector('#val').value === '9');
  await teacher.click('#controlToggle');
  await student.waitForFunction(() => !document.body.classList.contains('control-owner'));
  ...
  await isolated.waitForFunction(() => document.querySelector('#shareStatus')?.textContent.includes('Сессия учителя'));
  ...
  await student.waitForFunction(() => document.querySelector('#shareStatus')?.textContent === 'подключено');
  await waitMirror(student, 'connected');
  const reloadedStudentLinear = await trainerFrame(student, 'linear-inequalities-stepwise.html');
  await reloadedStudentLinear.waitForFunction(() => document.querySelector('#val').value === '9');
  ...
  await lateLinear.waitForFunction(() => document.querySelector('#val').value === '9');
  ...
  await oldStudent.waitForFunction(() => document.querySelector('#shareStatus')?.textContent === 'подключено');
  const oldNegative = await trainerFrame(oldStudent, 'negative-numbers-line.html');
  await teacherNegative.evaluate(() => { hint(); });
  await oldNegative.waitForFunction(() => window.getBoardTrainerState().hintVisible === true);
