(function () {
  ...
  /* объединённая линейка курса: задачи темы-приложения «Развёртки» (razv-*)
     идут сверх задания 3 — в отчёте это видно отдельно, как в навигаторе курса */
  function appendixNote(problems) {
    var extra = problems.filter(function (p) { return /^razv-/.test(p.id); }).length;
    return extra ? " (задание 3 — " + (problems.length - extra) + ", приложение «Развёртки» — " + extra + ")" : "";
  }
  ...
  function makeReport() {
    ...
    var lines = [
      ...
      "Имя ученика: ",
      "",
      "Всего задач: " + problems.length + appendixNote(problems),
      "Решено верно: " + solved,
