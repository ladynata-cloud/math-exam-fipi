...
try {
  ...
  mark('page-add-switch-delete-edge-legacy-snapshot');
  const legacyStudent = connectSocket();
  await join(legacyStudent, room, 'student', room.studentToken);
  const pageAddB = waitFor(studentLive, 'board:page-add', payload => payload?.page?.id === 'PageB01');
  ...
  const pageAddPayload = await pageAddB;
  assert.equal(pageAddPayload.page.bg, 'blank');
  assert.equal(pageAddPayload.page.strokes.length, 0);
  await legacyPageAddSnapshot;

  const pageAddC = waitFor(studentLive, 'board:page-add', payload => payload?.page?.id === 'PageC01');
  teacher.emit('board:page-add', roomPayload(room, room.teacherToken, {
    pageIndex: 2,
    page: { id: 'PageC01', bg: 'grid' }
  }));
  await pageAddC;
  current = await joinStudentState(room);
  assert.equal(current.pages.length, 3);
  assert.equal(current.currentPage, 2);

  const pageSwitchToB = waitFor(studentLive, 'board:page-switch', payload => payload?.pageIndex === 1);
  teacher.emit('board:page-switch', roomPayload(room, room.teacherToken, { pageIndex: 1 }));
  await pageSwitchToB;
  current = await joinStudentState(room);
  assert.equal(current.currentPage, 1);
  assert.equal(current.pages[current.currentPage].id, 'PageB01');

  const deleteA = waitFor(studentLive, 'board:page-delete', payload => payload?.pageIndex === 0 && payload?.currentPage === 0);
  const legacyDeleteSnapshot = waitFor(legacyStudent, 'board:snapshot', payload => (
    payload?.state?.currentPage === 0 && payload?.state?.pages?.[0]?.id === 'PageB01'
  ));
  ...
  assert.equal(current.pages[0].id, 'PageB01');
  assert.equal(current.pages[1].id, 'PageC01');
